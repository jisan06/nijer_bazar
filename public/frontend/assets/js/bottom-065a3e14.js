...
module.exports=g}),(function(module,exports,__webpack_require__){(function(global){module.exports=global.Tether=__webpack_require__(29)}.call(exports,__webpack_require__(30)))}),(function(module,exports,__webpack_require__){__webpack_require__(4);module.exports=__webpack_require__(5)})]);/*! fancyBox v2.1.5 fancyapps.com | fancyapps.com/fancybox/#license */
...
function countWord(cleanedWordString){var word_count=cleanedWordString.length-1;return word_count}}})(jQuery);function getCommentForm(){if(document.forms)
return(document.forms.comment_form);else return(document.comment_form)}
function getCommentDeleteForm(){if(document.forms)
return(document.forms.delete_comment_form);else return(document.delete_comment_form)}
function acceptComment(id){var form=getCommentForm();if(id)
form.elements.id_product_comment.value=id;form.elements.action.value='accept';form.submit()}
function deleteComment(id){var form=getCommentForm();if(id)
form.elements.id_product_comment.value=id;form.elements.action.value='delete';form.submit()}
function delComment(id,confirmation){var answer=confirm(confirmation);if(answer){var form=getCommentDeleteForm();if(id)
form.elements.delete_id_product_comment.value=id;form.elements.delete_action.value='delete';form.submit()}}
function getCriterionForm(){if(document.forms)
return(document.forms.criterion_form);else return(document.criterion_form)}
function editCriterion(id){var form=getCriterionForm();form.elements.id_product_comment_criterion.value=id;form.elements.criterion_name.value=document.getElementById('criterion_name_'+id).value;form.elements.criterion_action.value='edit';form.submit()}
function deleteCriterion(id){var form=getCriterionForm();form.elements.id_product_comment_criterion.value=id;form.elements.criterion_action.value='delete';form.submit()}
$(document).ready(function(){$('select#id_product_comment_criterion_type').change(function(){$('#categoryBox').closest('div.form-group').hide();$('#ids_product').closest('div.form-group').hide();$('#categories-treeview').closest('div.margin-form').hide();$('#categories-treeview').closest('div.margin-form').prev().hide();$('#ids_product').closest('div.margin-form').hide();$('#ids_product').closest('div.margin-form').prev().hide();if(this.value==2){$('#categoryBox').closest('div.form-group').show();$('#categories-treeview').closest('div.margin-form').show();$('#categories-treeview').closest('div.margin-form').prev().show()}else if(this.value==3){$('#ids_product').closest('div.form-group').show();$('#ids_product').closest('div.margin-form').show();$('#ids_product').closest('div.margin-form').prev().show()}});$('select#id_product_comment_criterion_type').trigger("change")});$(document).ready(function(){$('input.star').rating();$('.auto-submit-star').rating();$('.open-comment-form').fancybox({'autoSize':!1,'width':600,'height':'auto','hideOnContentClick':!1});var url_options='?';if(typeof(productcomments_url_rewrite)!=='undefined'){if(productcomments_url_rewrite=='0'){url_options='&'}}
...
function productcommentRefreshPage(){window.location.reload()};$(document).ready(function(){prestashop.blockcart=prestashop.blockcart||{};var showModal=prestashop.blockcart.showModal||function(modal){var $body=$('body');$body.append(modal);$body.one('click','#blockcart-modal',function(event){if(event.target.id==='blockcart-modal'){$(event.target).remove()}})};$(document).ready(function(){prestashop.on('updateCart',function(event){var refreshURL=$('.blockcart').data('refresh-url');var requestData={};if(event&&event.reason){requestData={id_product_attribute:event.reason.idProductAttribute,id_product:event.reason.idProduct,action:event.reason.linkAction}}
