			...
			<div class="card" style="margin-bottom: 0px;">
				...
				<div class="card-body">
					...
					<div class="modal-body">
						<form class="form-horizontal" action="{{ route('collectionHistory.index') }}" method="post" enctype="multipart/form-data">
							{{ csrf_field() }}
                                    ...
                                    <div class="form-group">
                                        <select class="form-control chosen-select" id="customer" name="customer[]" multiple>
                                            @foreach ($customers as $customerInfo)
                                                ...
                                                @endphp
                                                @if ($customerInfo->name == 'Cash')
                                                	<option value="{{ $customerInfo->name }}" {{ $cashSelect }}>{{ $customerInfo->name }}</option>
                                                @else
				...
				<div class="card-header">
					<div class="row">
						...
						<div class="col-md-6 text-right">
							<form class="form-horizontal" action="{{ route('collectionHistory.print') }}" target="_blank" method="post" enctype="multipart/form-data">
								{{ csrf_field() }}
						...
						<table id="dataTable" name="dataTable" class="table table-bordered">
                            ...
                            <tbody id="tbody">
                                ...
                                @foreach ($collectionHistories as $collectionHistory)
                                    ...
                                    <tr>
                                        <td>{{ $sl }}</td>
                                        <td>{{ Date('d-m-Y',strtotime($collectionHistory->date)) }}</td>
