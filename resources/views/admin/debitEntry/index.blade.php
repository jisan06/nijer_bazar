    ...
    <script>
        $(document).ready(function() {
            ...
            $('#dataTable tbody').on( 'click', 'i.fa-trash', function () {
                ...
                debitEntryId = $(this).parent().data('id');
                var tableRow = this;
                swal({   
                    title: "Are you sure?",   
                ...
                function(isConfirm){   
                    if (isConfirm) {
                        $.ajax({
                            type: "POST",
                            url : "{{ route('debitEntry.delete') }}",
                            data : {debitEntryId:debitEntryId},
                           
                            success: function(response) {
                                swal({
                                    title: "<small class='text-success'>Success!</small>", 
                                    type: "success",
                                    text: "Debit Voucher Deleted Successfully!",
                                    timer: 1000,
                                    html: true,
                                });
                                $('.row_'+debitEntryId).remove();
                            },
                    ...
                    { 
                        swal({
                            title: "Cancelled", 
                            type: "error",
                            text: "Your Debit Voucher is safe :)",
                            timer: 1000,
                            html: true,
                        });    
                    } 
        ...
        function statusChange(debitEntryId) {
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: "post",
                url: "{{ route('debitEntry.publish') }}",
                data: {debitEntryId:debitEntryId},
                success: function(response) {
                    swal({
                        title: "<small class='text-success'>Success!</small>", 
