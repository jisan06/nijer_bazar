...
class ProductWiseProfitController extends Controller
{
    public function index(Request $request)
    {
        ...
        $productWiseProfits = DB::table('product_wise_profit')
            ->select('categories.categoryName as categoryName','product_wise_profit.productId as productId','products.name as productName',DB::raw('SUM(product_wise_profit.cashProductQty) + SUM(product_wise_profit.creditProductQty) AS qty'), DB::raw('SUM(product_wise_profit.cashPriceAmount) + SUM(product_wise_profit.creditPriceAmount) as price'), DB::raw('SUM(product_wise_profit.cashVatAmount) + SUM(product_wise_profit.creditVatAmount) as vat'), DB::raw('SUM(product_wise_profit.cashDiscountAmount) + SUM(product_wise_profit.creditDiscountAmount) as discount'))
