...
class StockValuationReportController extends Controller
{
	public function index(Request $request)
	{
        ...
        $stockValuationReports = DB::table('stock_valuation_report')
            ->select('stock_valuation_report.categoryId','categories.categoryName as categoryName','stock_valuation_report.productId','products.name as productName', DB::raw('SUM(stock_valuation_report.cashSaleAmount) + SUM(stock_valuation_report.creditSaleAmount) as salesPrice'), DB::raw('((SUM(stock_valuation_report.cashPurchaseAmount) + SUM(stock_valuation_report.creditPurchaseAmount)) - SUM(stock_valuation_report.purchaseReturnAmount)) / ((SUM(stock_valuation_report.cashPurchaseQty) + SUM(stock_valuation_report.creditPurchaseQty)) - SUM(stock_valuation_report.purchaseReturnQty)) as avgLifting'), DB::raw('(SUM(stock_valuation_report.cashPurchaseQty) + SUM(stock_valuation_report.creditPurchaseQty) + SUM(stock_valuation_report.salesReturnQty)) - (SUM(stock_valuation_report.cashSaleQty) + SUM(stock_valuation_report.creditSaleQty) + SUM(stock_valuation_report.purchaseReturnQty)) as stockQty'))
	...
	public function print(Request $request)
	{
        ...
        $stockValuationReports = DB::table('stock_valuation_report')
            ->select('stock_valuation_report.categoryId','categories.categoryName as categoryName','stock_valuation_report.productId','products.name as productName', DB::raw('SUM(stock_valuation_report.cashSaleAmount) + SUM(stock_valuation_report.creditSaleAmount) as salesPrice'), DB::raw('(SUM(stock_valuation_report.cashPurchaseAmount) + SUM(stock_valuation_report.creditPurchaseAmount)) / (SUM(stock_valuation_report.cashPurchaseQty) + SUM(stock_valuation_report.creditPurchaseQty)) as avgLifting'), DB::raw('(SUM(stock_valuation_report.cashPurchaseQty) + SUM(stock_valuation_report.creditPurchaseQty) + SUM(stock_valuation_report.salesReturnQty)) - (SUM(stock_valuation_report.cashSaleQty) + SUM(stock_valuation_report.creditSaleQty) + SUM(stock_valuation_report.purchaseReturnQty)) as stockQty'))
            ->join('categories','categories.id','=','stock_valuation_report.categoryId')
            ->join('products','products.id','=','stock_valuation_report.productId')
            ->orWhere(function($query) use($category,$product){
                if (@$category)
